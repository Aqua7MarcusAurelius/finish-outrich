...
Промты живут в текстовых файлах в папке `prompts/` в корне репо:

```
prompts/
├── autochat_reply_system.md      — системный промт для ответов
└── autochat_initial_system.md    — системный промт для первого сообщения
```

Файлы читаются **при каждой генерации** — правишь файл, сохраняешь, следующий запрос в Opus использует новый текст. Без рестарта приложения.

### Плейсхолдеры

В текст промта можно вставить `{key}` — они подставятся кодом:

| Плейсхолдер | Где поддерживается | Что подставляется |
|---|---|---|
| `{current_time}` | оба | `2026-04-22 14:23` (UTC) |
| `{user_system_prompt}` | оба | из `autochat_sessions.system_prompt` (то что пришло в `POST /autochat/start`) |
| `{user_initial_prompt}` | только `initial` | из `autochat_sessions.initial_prompt` |
| `{conversation_history}` | только `reply` (опционально) | вся история диалога одним текстом — см. ниже |

HTML-комментарии `<!-- ... -->` в файлах промтов **вырезаются** перед отправкой в LLM — их можно использовать как заметки для редактора, в промт они не попадут.

### Режим `{conversation_history}` (для reply-промта)

По умолчанию история подаётся стандартно — через **role-based messages** (`{role: user, content: "..."}`, `{role: assistant, ...}`). LLM чётко видит кто что сказал.

Но если в `prompts/autochat_reply_system.md` встретится плейсхолдер `{conversation_history}` — код переключается в режим "всё в system": история подставляется туда текстом вида
...
и в messages идёт только `[system, user:"Ответь сейчас."]`. Удобно если хочется написать промт в стиле "Вот вся переписка: {conversation_history}. Теперь ты — {био}, ответь как будто ...".

Убрал плейсхолдер из файла — вернулся стандартный режим с ролями (обычно работает лучше, но менее гибко).

### Fallback

Если файл `prompts/autochat_*.md` не существует или не читается — в логе появится `autochat: prompt file not found, using fallback`, и будет использован минимальный встроенный дефолт (в `modules/autochat/generation.py::_REPLY_FALLBACK` / `_INITIAL_FALLBACK`). Система не падает, но ответы будут "скучными" — отредактируй файл.
...
Живёт прямо в файле `prompts/autochat_reply_system.md` — можно редактировать (см. раздел "Редактирование промтов" выше). Инструкция "оборачивай в `<msg>...</msg>`" должна остаться в любой версии промта — иначе парсер не найдёт сегменты и ответ уйдёт одним большим сообщением.

Парсер — регулярка `r"<msg>(.*?)</msg>"` с `re.DOTALL | re.IGNORECASE`. Если ни одного `<msg>` не найдено — фолбэк: весь ответ = один сегмент.

### Первое сообщение (initial)

Для `/autochat/start` промт берётся из `prompts/autochat_initial_system.md` — там инструкция "одно живое сообщение без `<msg>`-тегов". Если модель всё-таки вставила теги — `sanitize_initial_response()` их снимает и склеивает в одну строку.
...
```
prompts/
├── autochat_reply_system.md      — промт для ответов (редактируется вживую)
└── autochat_initial_system.md    — промт для первого сообщения
...
├── generation.py         — чтение промтов, сборка контекста, парсинг <msg>-сегментов
...
В `modules/worker/wrapper.py` — `resolve_username`, `set_typing`, `cancel_typing`, handler `_on_user_update`.

В `alembic/versions/` — новая миграция: таблица `autochat_sessions` + partial unique index + 7 настроек `autochat.*` в `settings` (все параметры задержек в БД, можно тюнить без рестарта).
